...
function feed2post_convertitem($item) {
  global $table_prefix, $wpdb;
	...
	if ( $item->id && ! $item->postid ) {
		...
		$handle = fopen(urldecode($item->link), "rb");
		$contents ='<a href="'.$item->link.'">'.$item->description.'</a>';
		...
		$post_data = array(
	  										...
	  										'post_content' => $wpdb->escape(feed2post_cleancontent($contents)),
  ...
  global $table_prefix, $wpdb;
	...
	$rss = fetch_rss( $url );
	  ...
	  foreach( $rss->items as $item) {
	  	...
	  	$sql = "SELECT guid FROM ".$table_prefix.FEED2POST_TABLENAME." WHERE description = '".$wpdb->escape($description)."'";
	  	...
	  	if ( !$guid ) {
			...
			if(trim($item['pubdate'])==""){
	  		...
	  		$sql = "INSERT INTO ".$table_prefix.FEED2POST_TABLENAME." (pubdate, title, link, guid, description, feedname, data) VALUES ('".date('Y-m-d H:i:s', strtotime($item['pubdate']))."', '".$wpdb->escape($title)."', '".$link."', '".$item['guid']."', '".$wpdb->escape($description)."', '".$wpdb->escape($url)."', '".$wpdb->escape(serialize($item))."')";
